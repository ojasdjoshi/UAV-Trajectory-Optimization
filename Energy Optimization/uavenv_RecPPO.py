...
class UavEnv(gym.Env):
    ...
    def _get_obs(self):
        """Construct the 15-feature observation vector for Recurrent PPO."""
        # 1. Spatial vectors (6 features)
        ...
        # 2. UAV Absolute Position (3 features)
        ...
        # 3. Dynamic metrics normalization (3 features)
        ...
        # 4. Temporal/Status indicators (3 features)
        progress = (2 * self.current_step / self.max_steps) - 1
        
        # Fetch current communication state to feed into memory
        # This helps the LSTM identify when it is entering a 'shadow' zone
        _, bottleneck_rate, _, _, _, _, _, _ = self._calculate_metrics()
        rate_norm = np.tanh(bottleneck_rate / 10) # Normalized throughput

        # User movement delta (helps LSTM predict where the user is going)
