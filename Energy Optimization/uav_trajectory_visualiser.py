...
class UAVTrajectoryVisualizer:
    ...
    Updated for Continuous Action Space and Variable Obstacle Heights.
    """
    ...
    def collect_trajectory(self, episode_num=0):
        ...
        # UPDATED: Store (x, y, height) for each of the 9 obstacles
        ...
        while not done and step < self.max_steps:
            # Predict the action from the trained model (supports continuous action)
            ...
            # UAV metrics calculation (since new env focuses on energy)
            curr_v = self.env.uav.get_velocity(1.0) # 1.0 is TIME_SLOT_DURATION
            ...
            # Mapping keys from new uavenv.py info dictionary
            ...
            # Rough velocity vector estimation from previous step
            vel_vec = np.array([
    ...
    def create_2d_animation(self, save_path="uav_trajectory_2d.gif", interval=100):
        ...
        # Visualize all obstacles pull from env
        for idx, obs in enumerate(self.obstacles):
            circle = Circle((obs[0], obs[1]), self.env.obstacle_radius, color='red', alpha=0.3, label='Obstacle' if idx == 0 else '')
        ...
        def animate(frame):
            ...
            step_text.set_text(f'Step:     {frame + 1:3d}\nSpeed:    {self.speeds[frame]:6.2f} m/s\nTotal Dst:{self.cumulative_distances[frame]:7.2f} m\nRate:     {self.data_rates[frame]:6.1f} kbps\nAlt:      {self.altitudes[frame]:6.1f} m')
            return uav_trail, uav_point, user_point, data_rate_line, altitude_line, speed_line, distance_line, step_text
    ...
    def create_3d_animation(self, save_path="uav_trajectory_3d.gif", interval=100):
        ...
        ax.set_title('UAV 3D Trajectory - Continuous Control', fontsize=14, fontweight='bold')
        
        ax.scatter([self.base_position[0]], [self.base_position[1]], [self.base_position[2]], c='green', marker='s', s=200, label='Base Station')
        
        # Render obstacles with their unique heights
        for obs in self.obstacles:
            theta = np.linspace(0, 2*np.pi, 20)
            z_cyl = np.linspace(0, obs[2], 2) # obs[2] is the unique height
        ...
        velocity_arrow = None
        step_text = fig.text(0.02, 0.95, '', fontsize=10, family='monospace', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.9))
    ...
    def create_static_plot(self, save_path="uav_trajectory_static.png"):
        """Create a comprehensive static plot of the mission."""
        print(f"  [→] Creating static trajectory plot...")
        fig = plt.figure(figsize=(20, 16))
        
        # 3D Overview
        ax1 = fig.add_subplot(3, 2, 1, projection='3d')
        ax1.set_title('3D Trajectory Overview', fontweight='bold')
        ...
        ax1.scatter([self.base_position[0]], [self.base_position[1]], [self.base_position[2]], c='green', marker='s', s=100)
        
        # Top Down View
        ax2 = fig.add_subplot(3, 2, 2)
        ax2.set_title('Top-Down Trajectory', fontweight='bold')
        ...
        # Performance Plots (Rate, Speed, Altitude, Distance)
    ...
    def visualize_all(self, output_dir="visualizations"):
        ...
        print(f"\n✓ Visualizations complete in '{output_dir}/'")
        self.env.close()
...
if __name__ == "__main__":
    ...
    if os.path.exists(MODEL_PATH):
        ...
        print(f"ERROR: Model not found at {MODEL_PATH}. Update the script path.")