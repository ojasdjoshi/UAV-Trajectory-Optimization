...
from sb3_contrib import RecurrentPPO  # Changed from SAC to RecurrentPPO
import os
import math
import argparse

# Import the environment we created
from uavenv_RecPPO import UavEnv, SPACE_X, SPACE_Y, SPACE_Z, MIN_ALTITUDE, MAX_ALTITUDE
...
class UAVTrajectoryVisualizer:
    ...
    Updated for Recurrent PPO Algorithm, Continuous Action Space, and Variable Obstacle Heights.
    """
    ...
    def __init__(self, model_path, num_episodes=3, max_steps=200):
        ...
        # Load the trained model - Changed to RecurrentPPO
    ...
    def collect_trajectory(self, episode_num=0):
        """
        Run one episode using the Recurrent PPO model and collect trajectory data.
        """
        self.reset_trajectory_data()
        ...
        # --- RECURRENT PPO SPECIFIC: Initialize LSTM States ---
        # The hidden state (lstm_states) must be None at the start of an episode
        lstm_states = None
        # episode_start is a boolean mask indicating the start of a sequence
        ...
        print(f"\n{'='*70}")
        print(f"   EPISODE {episode_num + 1}: RECURRENT PPO TRAJECTORY COLLECTION")
        print(f"{'='*70}")
        
        while not done and step < self.max_steps:
            # --- RECURRENT PPO SPECIFIC: Predict with Hidden States ---
            # We pass the obs, the current lstm_states, and the episode_start mask
            ...
            # After the first step, we are no longer at the "start" of the episode
    ...
    def create_2d_animation(self, save_path="uav_trajectory_2d.gif", interval=100):
        ...
        ax1.set_title('Recurrent PPO Trajectory - Top View', fontsize=13, fontweight='bold')
    ...
    def create_3d_animation(self, save_path="uav_trajectory_3d.gif", interval=100):
        ...
        ax.set_title('Recurrent PPO 3D Trajectory - LSTM Memory', fontsize=14, fontweight='bold')
    ...
    def create_static_plot(self, save_path="uav_trajectory_static.png"):
        ...
        ax1.set_title('3D Trajectory Overview (Recurrent PPO)', fontweight='bold')
        ...
        stats = (f'RECURRENT PPO MISSION STATISTICS\n'
    ...
    def visualize_all(self, output_dir="UAV_Optimization/visualizations"):
        """Run full visualization suite."""
        os.makedirs(output_dir, exist_ok=True)
        print(f"\n╔{'═'*68}╗")
        print(f"║   UAV RECURRENT PPO TRAJECTORY VISUALIZATION SYSTEM             ║")
        print(f"╚{'═'*68}╝\n")
        ...
        print(f"\n✓ All Recurrent PPO Visualizations complete in '{output_dir}/'")
        self.env.close()

if __name__ == "__main__":
    # Update this path to your actual trained Recurrent PPO model
    MODEL_PATH = r"Energy Optimization\models\recurrent_ppo\1772874713\recurrent_ppo_final_model.zip"
    
    parser = argparse.ArgumentParser(description='UAV Recurrent PPO visualizer')
    parser.add_argument('--model', type=str, default=MODEL_PATH, help='Path to RecurrentPPO model .zip file')
    ...
    if os.path.exists(args.model):
        ...
        print(f"ERROR: Recurrent PPO Model not found at {args.model}.")
        print("Please check your file path in the MODEL_PATH variable.")