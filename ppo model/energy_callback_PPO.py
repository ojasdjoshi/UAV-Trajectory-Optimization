...
class EnergyCallback(BaseCallback):
    """
    Custom callback for PPO — logs all metrics identical to the SAC and
    RecurrentPPO EnergyCallbacks for apples-to-apples TensorBoard comparison.

    Standard PPO wraps the env in a VecEnv internally, so:
    ...
    def _log_episode_metrics(self):
        ...
        if self.verbose > 0:
            print(f"\n=== Episode {self.episodes_finished + 1} Summary (PPO) ===")
