...
NUM_USERS = 1 
MAX_UAV_SPEED = 10.0 # Max meters per step for continuous reference (not used directly in discrete)
...
class Entity:
    ...
    def __init__(self, x=None, y=None, z=None):
        ...
        # Track velocity and distance (Maintained from original 320-line version)
    ...
    def action(self, choice):
        """Moves the entity based on a discrete action choice."""
        # Store previous position for distance calculation
        self.prev_x, self.prev_y, self.prev_z = self.x, self.y, self.z
        
        # Discrete move set: 
        # 0-3: Cardinal, 4-7: Diagonal, 8-9: Vertical, 10: Hover (Stay)
        moves = [
            (10, 0, 0), (-10, 0, 0), (0, 10, 0), (0, -10, 0),   # Cardinal
            (7, 7, 0), (-7, 7, 0), (7, -7, 0), (-7, -7, 0),    # Diagonal
            (0, 0, 10), (0, 0, -10),                           # Vertical
            (0, 0, 0)                                          # Hover
        ]
        
        dx, dy, dz = moves[choice]
        
        # Apply movement with clipping to stay within bounds
        self.x = np.clip(self.x + dx, 0, SPACE_X)
        self.y = np.clip(self.y + dy, 0, SPACE_Y)
        self.z = np.clip(self.z + dz, MIN_ALTITUDE, MAX_ALTITUDE)
        
        # Calculate actual displacement and velocity
        actual_dx = self.x - self.prev_x
        actual_dy = self.y - self.prev_y
        actual_dz = self.z - self.prev_z
...
class UavEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # --- DISCRETE ACTION SPACE (11 choices) ---
    ...
    def step(self, action):
        ...
        # Track distance traveled this step (Discrete Action)
        ...
        # Enhanced info dictionary with velocity and distance
        info = {
